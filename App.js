...
const data = [{name: 'Giovanny', cor: 'blue'}, {name: 'Ana', cor: 'green'}];

const handleKeyExtractor = item => item.name;
...
const HomeScreen = ({navigation}) => {
  const renderListItem = ({item}) => renderItem(item, navigation);

  return (
    <View style={styles.container}>
      {/* {lista.map(fruta => (
        <Text key={fruta} style={styles.item}>
          {fruta}
        </Text>
      ))} */}
      <FlatList
